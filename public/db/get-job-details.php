...
SELECT jp.ID,
    ...
    jp.JOB_TYPE,
    NVL(jp.JOB_TITLE, jp.JOB_DESCRIPTION) AS JOB_ROLE,
...
$response = [
    ...
    'job' => [
        ...
        // Accessibility & support fields (DB columns)
        'comp_req' => merge_text_and_profile_array($row['COMP_REQ'] ?? null, $profiles['comp_req'] ?? null) ?? ($row['COMP_REQ'] ?? null),
        'sensor_req' => merge_text_and_profile_array($row['SENSOR_REQ'] ?? null, $profiles['sensor_req'] ?? null) ?? ($row['SENSOR_REQ'] ?? null),
        'cog_lvl_req' => merge_text_and_profile_array($row['COG_LVL_REQ'] ?? null, $profiles['cog_lvl_req'] ?? null) ?? ($row['COG_LVL_REQ'] ?? null),
        'accom_avail' => merge_text_and_profile_array($row['ACCOM_AVAIL'] ?? null, $profiles['accom_avail'] ?? null) ?? ($row['ACCOM_AVAIL'] ?? null),
