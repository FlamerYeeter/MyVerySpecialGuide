...
try {
    ...
    try {
        if (!empty($guardian_id) && (!empty($job_id) || !empty($posted_job_id))) {
            ...
            $capSql = "INSERT INTO MVSG.JOB_CAPACITY (JOB_POSTING_ID, USER_ID, ROLE, CREATED_AT, UPDATED_AT, STATUS) VALUES (TO_NUMBER(:b_jid), TO_NUMBER(:b_uid), :b_role, SYSTIMESTAMP, SYSTIMESTAMP, 'Under Review')";
