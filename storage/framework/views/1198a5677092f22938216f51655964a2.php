...
<body class="bg-white flex justify-center items-start min-h-screen p-4 sm:p-6 md:p-8 relative overflow-x-hidden">
    ...
    onclick="(history.length>1 ? history.back() : window.location.href='<?php echo e(route('registerreview1')); ?>')">
    ...
    <div id="modalContent" class="p-2 text-center"></div>
            ...
            <div class="text-center mt-10">
                ...
                <button type="button"
                    ...
                    onclick="window.location.href='<?php echo e(route('registerreview4')); ?>'">
                    Continue →
                </button>
           ...
           <script>
            ...
            <script>
                ...
                document.addEventListener('DOMContentLoaded', function(){
                    try {
                        const routeMap = {
                            'education': '<?php echo e(route('registereducation')); ?>',
                            'work': '<?php echo e(route('registerworkexpinfo')); ?>',
                            'support': '<?php echo e(route('registersupportneed')); ?>',
                            'environment': '<?php echo e(route('registerworkplace')); ?>'
                        };
            ...
            <script src="<?php echo e(asset('js/firebase-config-global.js')); ?>"></script>
                ...
                <script>
                    ...
                    (async function(){
                        try {
                            ...
                            const resp = await fetch('<?php echo e(url('/debug/firebase-session')); ?>', { credentials: 'same-origin' });
            ...
            <?php endif; ?>
            <?php if(!empty($serverProfile)): ?>
                <script>
                    // Server-provided Firestore profile (admin route)
                    window.__mvsg_serverProfile = <?php echo json_encode($serverProfile, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
                    window.__mvsg_serverProfileUid = <?php echo json_encode($serverProfileUid ?? null); ?>;
                </script>
            <?php endif; ?>
            <script>
                ...
                document.addEventListener('DOMContentLoaded', async () => {
                    ...
                    async function trySyncClientIdTokenToServer() {
                        try {
                            ...
                            const resp = await fetch('<?php echo e(url('/session/firebase-signin')); ?>', { method: 'POST', credentials: 'same-origin', headers, body: JSON.stringify({ idToken }) });
                    ...
                    try {
                        ...
                        if (!data) {
                            ...
                            try {
                                const resp = await fetch('<?php echo e(url('/api/server-profile')); ?>', { credentials: 'same-origin' });
                                if (resp && resp.ok) {
                                    const json = await resp.json().catch(() => null);
                                    ...
                                    try {
                                        const dbg = await fetch('<?php echo e(url('/debug/firebase-session')); ?>', { credentials: 'same-origin' });
                                        ...
                                        if (dbgJson && dbgJson.session_firebase_uid) {
                                            try {
                                                const overrideResp = await fetch('<?php echo e(url('/api/server-profile')); ?>?uid=' + encodeURIComponent(dbgJson.session_firebase_uid), { credentials: 'same-origin' });
...
<?php /**PATH C:\xampp\htdocs\MyVerySpecialGuide\resources\views/ds_register_review-2.blade.php ENDPATH**/ ?>