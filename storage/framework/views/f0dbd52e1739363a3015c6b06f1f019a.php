                ...
                <?php else: ?>
                    <?php $__currentLoopData = $recommendations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $job): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <?php
                            ...
                            if (is_numeric($normContent) && $normContent > 0 && $normContent <= 1.01) {
                                $normContent = $normContent * 100.0;
                            }
                            ...
                            $rawTitle = trim(
                                strval(
                                    $job['title'] ?? ($job['job_title'] ?? ($job['Title'] ?? ($job['JobTitle'] ?? ''))),
                                ),
                            );
                            if ($rawTitle === '') {
                                ...
                                if (
                                    preg_match(
                                        '/^(.{1,140}?)(?:\.|\n| needed with| needed| required with| required|:| - )/i',
                                        $desc,
                                        $m,
                                    )
                                ) {
                            ...
                            $companyName = trim(
                                strval($job['company'] ?? ($job['company_name'] ?? ($job['Company'] ?? ''))),
                            );
                            ...
                            if (!empty($computedMaxScore)) {
                                ...
                                if (is_numeric($rawMatchDisplay) && is_numeric($computedMaxScore)) {
                                    $rawMatchDisplay =
                                        (string) $rawMatchDisplay . ' (max: ' . (string) $computedMaxScore . ')';
                                }
                            }
                                        ...
                                        <?php
                                            ...
                                            try {
                                                $skillTags = is_string($job['skills_desc'])
                                                    ? array_filter(
                                                        array_map('trim', preg_split('/[,;|]+/', $job['skills_desc'])),
                                                    )
                                                    : [];
                                            } catch (Exception $e) {
                                                $skillTags = [];
