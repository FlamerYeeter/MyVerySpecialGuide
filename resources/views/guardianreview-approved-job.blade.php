...
<!-- HEADER BANNER: NOTE AAYUSIN KO PA MAMAYA -->
  ...
  <div class="max-w-5xl mx-auto mt-10 px-4">
    @php
        // Reuse the same CSV loader + approvals map as pending view, then filter for approved
    ...
    if (!empty($uidParam)) {
      try {
        ...
        logger()->warning('Firestore fetch failed in approved view: ' . $e->getMessage());
      }
    }
    if (empty($guardianApprovals)) {
      ...
      if (file_exists($approvals_path)) $guardianApprovals = json_decode(file_get_contents($approvals_path), true) ?: [];
        ...
        $approved = array_values(array_filter($jobs, function($j) use ($guardianApprovals) {
            $id = (string)$j['job_id'];
            return isset($guardianApprovals[$id]) && (($guardianApprovals[$id]['status'] ?? '') === 'approved');
        }));
        usort($approved, function($a,$b){ return intval($b['match_score'] ?? 0) <=> intval($a['match_score'] ?? 0); });
        $total = count($approved);
        $perPage = 10; $page = max(1,intval(request()->query('page',1))); $lastPage = max(1,(int)ceil($total/$perPage));
        if ($page > $lastPage) $page = $lastPage;
        $start = ($page - 1) * $perPage; $paged = array_slice($approved, $start, $perPage);
    @endphp

    <h3 class="text-lg font-bold text-green-600 mb-4">Approved Job: {{ $total }} · Page {{ $page }} / {{ $lastPage }}</h3>

    @foreach($paged as $p)
      <div id="job-card-{{ $p['job_id'] }}" class="border-2 border-green-400 rounded-2xl p-6 bg-green-50/20 shadow-sm mb-6">
        <div class="flex justify-between items-start">
          <div class="flex items-center space-x-2">
            <img src="/images/job-icon.png" class="w-6 h-6" alt="Job Icon">
            <h4 class="text-lg font-semibold">{{ $p['title'] }}</h4>
          </div>
          <span class="bg-green-200 text-green-800 px-3 py-1 rounded-full text-xs font-semibold">Approved</span>
        </div>
        <div class="mt-4"><span class="bg-green-200 text-green-800 px-3 py-1 rounded-md text-sm font-semibold">{{ $p['match_score'] }}% Match</span></div>
