...
<body class="bg-white flex justify-center items-center min-h-screen p-3 sm:p-6 relative overflow-x-hidden">
    ...
    <!-- Back Button -->
    <button
        class="fixed left-2 top-2 sm:left-5 sm:top-5 bg-blue-600 text-white px-3 sm:px-5 md:px-6 py-2 sm:py-3 rounded-2xl flex items-center gap-1 sm:gap-2 text-xs sm:text-base font-semibold shadow-lg hover:bg-blue-700 active:scale-95 transition z-50"
    ...
    <div
        class="bg-[#FEF2C7] w-full max-w-6xl rounded-3xl shadow-2xl p-4 sm:p-8 md:p-12 relative z-10 border-4 border-blue-200 mx-auto">
        ...
        <div class="mt-8 text-center sm:text-left px-1 sm:px-4">
            <h3 class="text-lg sm:text-2xl font-bold text-blue-700 mb-2">Education</h3>
            <div class="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 sm:gap-2">
                <p class="text-sm sm:text-lg font-medium text-gray-800">What is your highest education?</p>
                <button class="text-gray-500 text-lg sm:text-2xl hover:scale-110 transition-transform">🔊</button>
            </div>
            <p class="text-gray-600 italic text-xs sm:text-sm mt-1">(Ano ang pinakamataas mong natapos na grade o taon
                sa school?)</p>
        </div>
        ...
        <div class="mt-6 text-center sm:text-left px-1 sm:px-4">
            <div class="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1 sm:gap-2">
                <p class="text-xs sm:text-base font-medium text-gray-800">Choose from the pictures provided and click
                    your answer.</p>
                <button class="text-gray-500 text-lg sm:text-2xl hover:scale-110 transition-transform">🔊</button>
            </div>
        ...
        <div class="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 mt-8 px-2 sm:px-4">
            ...
            <div class="education-card bg-white p-4 sm:p-5 rounded-2xl hover:bg-blue-100 hover:shadow-xl transition cursor-pointer relative flex flex-col justify-between text-center col-span-1 sm:col-span-2 lg:col-span-1"
                ...
                <p class="text-xs sm:text-sm text-gray-700 mt-1">Type your answer inside the box if not in the choices
                </p>
                <p class="text-[10px] sm:text-xs text-gray-500 italic mt-1 mb-2">(Isulat ang sagot sa loob ng kahon kung
    ...
    <script src="{{ asset('js/register.js') }}"></script> <!-- Make selectEducationChoice available globally for inline onclick handlers -->
    <script>
        // filepath: c:\xampp\htdocs\MyVerySpecialGuide\resources\views\ds_register_education.blade.php function 
        selectEducationChoice(el, value) {
            try {
