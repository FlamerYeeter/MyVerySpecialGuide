        ...
        (function() {
            ...
            function createUploadSlotForNode(targetNode) {
                ...
                inp.addEventListener('change', function() {
                    ...
                    if (!['jpg','jpeg','png','pdf'].includes(type)) { try{ showOcrModal({ type: 'error', title: 'Invalid file', message: 'Invalid file type. Only JPG, PNG, or PDF allowed.', details: [], confirmText: 'OK' }); }catch(e){ alert('Invalid file type. Only JPG, PNG, or PDF allowed.'); } this.value=''; return; }
                    if (f.size > 5 * 1024 * 1024) { try{ showOcrModal({ type: 'error', title: 'File too large', message: 'File too large. Max 5MB.', details: [], confirmText: 'OK' }); }catch(e){ alert('File too large. Max 5MB.'); } this.value=''; return; }
                    const reader = new FileReader();
                ...
                fileInput.addEventListener("change", async function() {
                    ...
                    for (const f of files) {
                        ...
                        if (!["jpg", "jpeg", "png", "pdf"].includes(ext)) {
                            try{ showOcrModal({ type: 'error', title: 'Invalid file', message: 'Invalid file type. Only JPG, PNG, or PDF allowed.', details: [], confirmText: 'OK' }); }catch(e){ alert("Invalid file type. Only JPG, PNG, or PDF allowed."); }
                            continue;
                        }
                        if (f.size > MAX_BYTES) {
                            try{ showOcrModal({ type: 'error', title: 'File too large', message: `${f.name} is too large. Max file size is 5MB.`, details: [], confirmText: 'OK' }); }catch(e){ alert(`${f.name} is too large. Max file size is 5MB.`); }
                            continue;
                        }
                        try {
    ...
    <script>
                        ...
                        try{
                            ...
                            .then(res => {
                                if (res.ok) {
                                    ...
                                    try {
                                        ...
                                        if (detected === 'certificate_proof') {
                                            ...
                                            try {
                                                ...
                                                // Show OCR result modal with key fields for confirmation
                                                try {
                                                    showOcrModal({
                                                        type: 'success',
                                                        ...
                                                        details: [
                                                            { label: 'Certificate', value: ai.cert_name || ai.name || '' },
                                                            { label: 'Issuer', value: ai.issued_by || ai.issuer || '' },
                                                            { label: 'Date', value: (ai.date_completed || ai.date || '') }
                                                        ],
                                            ...
                                            // other OCR types — show non-intrusive confirmation modal (no OCR Type displayed)
                                            try {
                                                showOcrModal({
                                                    type: 'success',
                                                    title: 'Document Processed',
                                                    message: (detected ? ('Processed: ' + detected) : 'Document processed successfully.'),
                                                    details: [],
                                                    confirmText: 'Confirm & Continue'
                                                });
                                            } catch(e) {
                                                // Fallback to alert if modal unavailable
                                                try{ showOcrModal({ type: 'success', title: 'Document Processed', message: 'Document processed successfully.', details: [], confirmText: 'Confirm & Continue' }); } catch(err){ alert('Document processed successfully.'); }
                                            }
                                        }
                                    ...
                                    // ❌ Error
                                        try{ showOcrModal({ type: 'error', title: 'Scan Failed', message: `Error ${res.status}: ${res.body.message || 'Unknown error'}`, details: [], confirmText: 'OK' }); } catch(err){ alert(`Error ${res.status}: ${res.body.message || 'Unknown error'}`); }
                                }
                            })
                            .catch(err => {
                                try{ const ld = document.getElementById(loadingId); if(ld) ld.remove(); }catch(e){}
                                console.error('Fetch error:', err);
                                    try{ showOcrModal({ type: 'error', title: 'Network Error', message: 'Failed to fetch OCR data.', details: [], confirmText: 'OK' }); } catch(err2){ alert('Failed to fetch OCR data.'); }
                            });  

                        }catch(err){ console.warn(err); }
                    };
                    reader.onerror = function(){ try{ showOcrModal({ type: 'error', title: 'Read Error', message: 'Failed to read file.', details: [], confirmText: 'OK' }); }catch(e){ alert('Failed to read file'); } };
                    reader.readAsDataURL(f);
                });
        ...
        (function(){
            ...
            window.showOcrModal = function(opts) {
                ...
                messageEl.textContent = opts.message || (type === 'error' ? 'We couldn’t complete the scan.' : 'We’ve successfully processed the uploaded document.');
            ...
            window.closeOcrModal = function() {
                const modal = document.getElementById('ocrResultModal'); if (!modal) return; setModalVisibility(modal, false);
            };

            document.addEventListener('DOMContentLoaded', function() {
                const modal = document.getElementById('ocrResultModal'); if (!modal) return;
                modal.addEventListener('click', function(event) { if (event.target === modal) closeOcrModal(); });
                const closeBtn = document.getElementById('ocrModalCloseBtn'); if (closeBtn) closeBtn.addEventListener('click', closeOcrModal);
                const primaryBtn = document.getElementById('ocrModalPrimaryBtn');
                if (primaryBtn) {
                    primaryBtn.addEventListener('click', function() {
                        const action = this.dataset.action; closeOcrModal(); if (action === 'retry') document.dispatchEvent(new CustomEvent('ocrRetryRequested'));
                    });
                }
            });
        })();
