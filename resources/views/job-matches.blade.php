...
<section class="max-w-6xl mx-auto mt-10 bg-[#00ae7a] rounded-md p-6">
  ...
  <script>
    ...
    <div class="max-w-6xl mx-auto px-6 space-y-8 mb-20">
        {{-- New dynamic recommendations UI: show CBF-based jobs (Pet Care Assistant design) and Recommended Job/company suggestions --}}
        @php
            // Use variables provided by the controller/route. Fallback to empty arrays if not present.
            $cbfList = $cbfList ?? [];
            $topCompanies = $topCompanies ?? [];
        @endphp

        {{-- Pet Care Assistant — content-based matches (visual card strip) --}}
        <section class="bg-white rounded-md p-6 shadow-sm">
            <h4 class="text-lg font-semibold">Pet Care Assistant — Jobs Matching Your Skills (Content-based)</h4>
            <p class="text-sm text-gray-600">These jobs are selected using content-based matching of your profile to job descriptions.</p>

            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                @forelse($cbfList as $cidx => $job)
                    @php
                        $jid = $job['id'] ?? ($job['job_id'] ?? ('p' . $cidx));
                        $title = $job['title'] ?? ($job['Title'] ?? 'Untitled Job');
                        $company = $job['company'] ?? ($job['Company'] ?? '');
                        $snippet = \Illuminate\Support\Str::limit($job['data']['description'] ?? $job['data']['job_description'] ?? '', 220);
                        $cbfRaw = is_numeric($job['cbf']) ? floatval($job['cbf']) : 0.0;
                        $cbfPct = $cbfRaw * 100.0;
                        if ($cbfPct > 0 && $cbfPct < 1) {
                            $cbfDisplay = '&lt;1';
                        } else {
                            $cbfDisplay = is_numeric($cbfPct) ? round($cbfPct, 1) : 0;
                        }
                    @endphp
                    <div class="border p-4 rounded-md flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-sm mt-2 text-gray-700">{{ $snippet }}</p>
                            <div class="mt-3"><span class="bg-green-100 text-green-800 px-3 py-1 rounded">CBF: {!! $cbfDisplay !!}% Match</span></div>
                        </div>
                        <div class="flex flex-col items-end">
                            <a href="{{ route('job.details', ['job_id' => $jid]) }}" class="bg-blue-500 text-white px-4 py-2 rounded-md text-sm">View</a>
                            <form method="POST" action="{{ route('my.job.applications') }}" class="mt-2">
                                @csrf
                ...
                @empty
                    <div class="text-gray-600">No content-based matches available yet. Please sign in and complete your profile.</div>
                @endforelse
            </div>
        </section>

        {{-- Recommended Job / Companies block (collaborative summary) --}}
        <section class="mt-6 bg-white p-6 rounded-md shadow-sm">
            <h4 class="text-lg font-semibold">Recommended Job</h4>
            <p class="text-sm text-gray-600">Recommended companies based on application history, preferences, and recent platform activity.</p>
            <div class="mt-4 flex gap-3 flex-wrap">
                @forelse($topCompanies ?? [] as $comp)
                    <span class="bg-gray-100 px-4 py-2 rounded-md">{{ $comp }}</span>
                @empty
                    <p class="text-gray-600">No company recommendations available yet.</p>
                @endforelse
            </div>
        </section>

        {{-- Continue with job list below (server-rendered hybrid recommendations) --}}
                @foreach($recommendations as $idx => $job)
                @php
                    // Avoid undefined index when job_id is missing: evaluate with null-coalesce first
                    $job_dom_id = 'job_' . ($job['job_id'] ?? ('p' . ((($page - 1) * $perPage) + $idx)));
                @endphp
                @php
                ...
                @endphp
                    ...
                    <div>
                        ...
                        <div class="flex gap-2 mt-3 text-xs">
                            @if(!empty($job['fit_level']))<span class="bg-[#D1FFD6] text-green-800 px-3 py-1 rounded-md">⭐ {{ $job['fit_level'] }}</span>@endif
                            @if(!empty($job['growth_potential']))<span class="bg-[#E6E9FF] text-[#4F46E5] px-3 py-1 rounded-md">📈 {{ $job['growth_potential'] }}</span>@endif
                        </div>
    ...
    try {
        ...
        async function pollOnce() {
            ...
            try {
                ...
                // expose pollOnce to global scope so other modules (and the Firestore listener)
                // can trigger it directly. This fixes cases where pollOnce is not visible
                // to module-level callers due to function scoping.
                try { window.pollOnce = pollOnce; window.__pollOnceAvailable = true; } catch(e){}
                pollOnce();
                setInterval(pollOnce, pollIntervalMs);

                // Trigger a refresh when the page becomes visible (user switched back to tab)
                try {
                    window.addEventListener('visibilitychange', function(){ if (document.visibilityState === 'visible') { try { if (typeof pollOnce === 'function') pollOnce(); } catch(e){} } });
                    // pageshow fires on navigation via back/forward and some reloads
                    window.addEventListener('pageshow', function(){ try { if (typeof pollOnce === 'function') pollOnce(); } catch(e){} });
                } catch(e) { console.debug('attach visibility/pageshow handlers failed', e); }
    } catch (e) { console.debug('polling setup failed', e); }
})();
</script>
    <!-- Firestore application listener: subscribe to user's application docs and trigger recommendation refresh -->
    <script type="module">
    (async function(){
        try {
            // small status node to help debug subscription+refresh behavior
            const ensureStatusNode = () => {
                let st = document.getElementById('reco-listener-status');
                if (!st) {
                    st = document.createElement('div');
                    st.id = 'reco-listener-status';
                    st.style.cssText = 'position:fixed;right:12px;bottom:12px;z-index:9999;padding:8px 10px;background:#111;color:#fff;border-radius:6px;font-size:12px;opacity:0.9';
                    st.textContent = 'reco: init';
            ...
            const mod = await import("{{ asset('js/job-application-firebase.js') }}");
            statusNode.textContent = 'reco: module loaded';
            // Attempt to sign in with server token so the client auth state is available for the subscription
            try { const signed = await mod.signInWithServerToken("{{ route('firebase.token') }}"); statusNode.textContent = signed ? 'reco: signed' : 'reco: signed?false'; } catch (e) { statusNode.textContent = 'reco: signin failed'; }
            if (typeof mod.subscribeToApplications === 'function') {
                statusNode.textContent = 'reco: subscribing';
                const unsub = await mod.subscribeToApplications(async (docs, snap) => {
                    try {
                        const count = docs ? docs.length : (snap && typeof snap.size === 'number' ? snap.size : 0);
                        console.debug('Firestore applications changed (count=' + count + ')');
                        statusNode.textContent = 'reco: apps changed (' + count + ')';
                        // Force generation on the server (ask it to regenerate per-user cache)
                        try {
                            const p = await fetch('{{ url('/api/recommendations/user') }}', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: JSON.stringify({ force: true }) });
                            if (p && p.ok) {
                                statusNode.textContent = 'reco: generation requested';
                                // after server generation accepted, also try to rebuild client if pollOnce available
                                try { if (typeof pollOnce === 'function') pollOnce(); } catch(e) { console.debug('post-generate pollOnce failed', e); }
                            } else if (p && p.status === 202) {
                                statusNode.textContent = 'reco: generation queued';
                            } else {
                                statusNode.textContent = 'reco: generation failed';
                            }
                        } catch (e) {
                            console.debug('generation POST failed', e);
                            statusNode.textContent = 'reco: generation post err';
                        }
                        // update last-change time
                        try { const t = new Date().toLocaleTimeString(); statusNode.title = 'last change: ' + t; } catch(e){}
                    } catch (e) { console.debug('applications subscribe callback error', e); statusNode.textContent = 'reco: callback err'; }
                });
                // make unsubscribe available for debugging/testing
                try { window.__unsubscribeApplications = unsub; } catch (e) {}
                statusNode.textContent = 'reco: subscribed';
            } else {
                statusNode.textContent = 'reco: subscribe helper missing';
                console.debug('subscribeToApplications not available; falling back to HTTP polling (existing watcher)');
            }
        } catch (e) {
            console.debug('applications listener setup failed', e);
            try { const st = document.getElementById('reco-listener-status'); if (st) st.textContent = 'reco: setup fail'; } catch(e){}
        }
    })();
    </script>
<script>
    // Non-blocking automatic generation: if the server did not render per-user recommendations,
    // trigger the recommendations API once on page load and render returned results in-place.
    document.addEventListener('DOMContentLoaded', async function() {
        try {
            if (window.__SERVER_RECO_LOADED) return; // server already provided results
            const key = 'auto_reco_trigger_v2';
            if (sessionStorage.getItem(key)) return; // avoid repeated triggers per-tab
            sessionStorage.setItem(key, '1');

            // Show a small status node above the job list (non-blocking)
            const container = document.querySelector('.container.mx-auto.mt-8.px-4.space-y-6');
            let statusEl = document.getElementById('reco-auto-status');
            if (!statusEl) {
                statusEl = document.createElement('div');
                statusEl.id = 'reco-auto-status';
                statusEl.className = 'text-center text-sm text-gray-600 my-4';
                statusEl.textContent = 'Generating personalized recommendations...';
            }
            if (container && container.parentNode && !document.getElementById('reco-auto-status')) container.parentNode.insertBefore(statusEl, container);
            ...
            if (!resp) {
                if (statusEl) statusEl.textContent = 'Could not contact recommendation service.';
                return;
            }

            if (resp.status === 202) {
                if (statusEl) statusEl.textContent = 'Recommendations scheduled; they will appear shortly.';
                return;
            }

            if (resp.ok) {
                const data = await resp.json().catch(() => null);
                // Normalize to array similar to other client code
                const normalize = (d) => {
                    if (!d) return [];
                    if (Array.isArray(d)) return d;
                    if (typeof d === 'object') {
                ...
                if (Array.isArray(recs) && recs.length > 0) {
                    // Render similarly to existing render helpers (lightweight, safe escaping)
                    const esc = s => { if (s === null || s === undefined) return ''; return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); };
                    if (container) {
                        let out = '';
                        recs.slice(0,50).forEach((r2, idx) => {
                            ...
                            const deadline = esc(r2.deadline ?? '');
                            out += `
                                <div id="job_${jid}" data-job-id="${jid}" class="job-card bg-white shadow-md rounded-xl p-6 flex flex-col md:flex-row justify-between items-start">
                                    <div class="flex-1 pr-6">
                                        <h3 class="text-lg font-bold">${title}</h3>
                                        <div class="mt-2"><span class="js-match-badge bg-green-100 text-green-800 px-3 py-1 rounded-md text-sm font-semibold">${matchPercent}% Match <small class="text-xs text-gray-500">(raw: ${esc(String(rawMatchVal))})</small></span></div>
                                        ${ company ? `<p class="text-sm text-gray-700 font-medium">${company}</p>` : '' }
                        ...
                        container.innerHTML = out;
                        // mark that client rendered per-user recs so other scripts skip re-fetching
                        window.__SERVER_RECO_LOADED = true;
                    }
                } else {
                    if (statusEl) statusEl.textContent = 'No personalized recommendations returned yet.';
                    // allow background polling (existing poll code) to pick up later
                }
            } else {
                if (statusEl) statusEl.textContent = 'Recommendation generation failed or returned no data.';
            }

            if (statusEl) setTimeout(() => { try { statusEl.remove(); } catch(_) {} }, 3000);
        } catch (e) {
            console.debug('auto reco failed', e);
        }
    });
</script>
@endsection
